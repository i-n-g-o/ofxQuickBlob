...
void quickblob::blank(struct blob* b)
{
    b->size = 0;
    b->color = -1;
    b->x1 = -1;
...
int quickblob::init_pixel_stream(struct stream_state* stream)
{
    memset(stream, 0, sizeof(struct stream_state));
    if (init_pixel_stream_hook(stream))
        {return 1;}
	
    stream->row = NULL;
    // row is a bunch of 1x8 bit grayscale samples
	if (!destructive) {
		// allocation memory for one row
//		printf("allocating memory for one row\n");
...
int quickblob::close_pixel_stream(struct stream_state* stream)
{
    close_pixel_stream_hook(stream);  // TODO return status
	if (!destructive) {
		free(stream->row);
	}
    stream->row = NULL;
...
int quickblob::malloc_blobs(struct blob_list* blist)
{
...
int quickblob::init_blobs(struct blob_list* blist)
{
    int i, len;
    struct blob* head;
    len = blist->length;
...
void quickblob::blob_unlink(struct blob* b2)
...
void quickblob::blob_insert(struct blob* bl_start, struct blob* b2)
...
int quickblob::next_row(struct stream_state* stream)
{
    ...
    stream->x = 0;
    stream->y++;
    return next_row_hook(stream);
}

int quickblob::next_frame(struct stream_state* stream)
{
    stream->wrap = 0;
    stream->x = 0;
    stream->y = -1;
...
int quickblob::scan_segment(struct stream_state* stream, struct blob* b)
...
void quickblob::bbox_update(struct blob* b, int x1, int x2, int y1, int y2)
{
    if (b->bb_x1 < 0)
        {b->bb_x1 = x1;}
    if (x1 < b->bb_x1)
...
void quickblob::blob_update(struct blob* b, int x1, int x2, int y)
{
    int s2;
    s2  = 1 + x2 - x1;
    b->x1 = x1;
...
void quickblob::sib_link(struct blob* b1, struct blob* b2)
...
void quickblob::blob_merge(struct blob* b1, struct blob* b2)
...
int quickblob::range_overlap(int a1, int a2, int b1, int b2)
...
int quickblob::blob_overlap(struct blob* b, int x1, int x2)
...
void quickblob::blob_reap(struct blob_list* blist, struct blob* b)
{
    blob_unlink(b);
    blank(b);
...
void quickblob::sib_cleanup(struct blob_list* blist, struct blob* b)
...
void quickblob::sib_find(struct blob* blob_start, struct blob* blob_now)
{
    struct blob* b = NULL;
    int i;
    b = blob_start;
...
void quickblob::flush_incremental(struct blob_list* blist, struct blob* blob_now)
...
void quickblob::flush_old_blobs(struct blob_list* blist, int y)
// merges (or prints) and reaps, y is current row
{
    ...
    while (b)
    {
        ...
        if (b->prev != NULL)
            {b2 = b->prev;}
        if (b->sib_p == NULL && b->sib_n == NULL)
            {log_blob_hook(b); blob_reap(blist, b);}
        else
            {sib_cleanup(blist, b);}
        b = b2->next;
    }
...
struct blob* quickblob::empty_blob(struct blob_list* blist)
{
    struct blob* b;
    blist->empty_i--;
...
int quickblob::init(bool _destructive)
{
	destructive = _destructive;
	
	if (init_pixel_stream(&stream)) {
		printf("init malloc error!\n");
		return 1;
	}
	if (!destructive && stream.row == NULL) {
